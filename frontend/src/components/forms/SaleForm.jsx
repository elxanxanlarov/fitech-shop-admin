...
export default function SaleForm() {
        ...
        const fetchProducts = async () => {
            ...
            try {
                ...
                if (response.success && (response.data || response.date)) {
                    ...
                    // Yeni satış üçün: yalnız aktiv və stokda olan məhsullar
                    // Edit modunda: bütün məhsullar (stokda olmasa və ya qeyri-aktiv olsa belə, seçilmiş məhsul görünsün deyə)
                    if (isEditMode) {
                        setProducts(list);
                    } else {
                        const activeProducts = list.filter(p => p.isActive && p.stock > 0);
    ...
    useEffect(() => {
        const fetchCreditTerms = async () => {
            try {
                ...
                if (response.success && (response.data || response.date)) {
                    setCreditTerms(response.data || response.date);
                }
            } catch (error) {
                    ...
                    if (response.success && (response.data || response.date)) {
                        ...
                        if (sale.isCredit && sale.id) {
                            try {
                                ...
                                if (paymentsResponse.success && (paymentsResponse.data || paymentsResponse.date)) {
                                    setCreditPayments(paymentsResponse.data || paymentsResponse.date);
                                }
                            } catch (error) {
                        ...
                        if (sale.returns && sale.returns.length > 0) {
                            ...
                            if (returnsResponse.success && (returnsResponse.data || returnsResponse.date)) {
                                setSaleReturns(returnsResponse.data || returnsResponse.date);
                            }
                        }
                    }
                } catch (error) {
    ...
    const handleBoxesChange = (index, boxes) => {
        ...
        if (boxesNum > maxBoxes) {
            setErrors(prev => ({
                ...prev,
                [`quantity_${index}`]: `Maksimum ${maxBoxes} ${product.unitType === 'BOX' ? 'qutu' : 'paket'} seçə bilərsiniz`
            }));
            return;
        }
    ...
    const handleReturnBoxesChange = (saleItemId, boxes) => {
        ...
        if (finalTotalQuantity > availableReturnQuantity) {
            setReturnErrors(prev => ({
                ...prev,
                [saleItemId]: t('return_quantity_exceeds', { available: availableReturnQuantity }) || `Mövcud qaytarıla bilən: ${availableReturnQuantity} məhsul`
            }));
            return;
        }
        ...
        if (existingIndex >= 0) {
            const newItems = [...returnItems];
            newItems[existingIndex].boxes = finalBoxes;
            newItems[existingIndex].quantity = finalTotalQuantity;
            setReturnItems(newItems);
        } else {
    ...
    const handleReturnPiecesChange = (saleItemId, pieces) => {
        ...
        if (existingIndex >= 0) {
            const newItems = [...returnItems];
            newItems[existingIndex].pieces = finalPieces;
            newItems[existingIndex].quantity = finalTotalQuantity;
            setReturnItems(newItems);
        } else {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isEditMode) {
                ...
                if (response.success && (response.data || response.date)) {
                    const created = response.data || response.date;
                    ...
                    setTimeout(() => {
                        const checkPath = isAdmin ? `/admin/check?id=${created.id}` : `/reception/sale-form?id=${created.id}`;
                        navigate(checkPath);
                    }, 1500);
                } else {
                    setTimeout(() => {
                                                ...
                                                <SearchDropdown
                                                    ...
                                                    getOptionLabel={(product) => {
                                                        ...
                                                        if (product.unitType === 'PIECE') {
                                                            ...
                                                            let display = '';
                                                            if (fullBoxes > 0) {
                                                                display = `${fullBoxes} ${boxUnit}`;
                                                            }
                                                            if (openedQuantity > 0) {
                                                                if (display) display += ` + ${openedQuantity} açıq`;
                                                        ...
                                                        {selectedProduct && (
                                                            <p className={`mt-1 text-xs font-medium ${availableStock <= 0 ? 'text-red-600' : 'text-gray-500'}`}>
                                                                {(() => {
                                                                    ...
                                                                    let stockDisplay = '';
                                                                    if (fullBoxes > 0) {
                                                                        stockDisplay = `${fullBoxes} ${boxUnit}`;
                                                                    }
                                                                    if (openedQuantity > 0) {
                                                                        if (stockDisplay) stockDisplay += ` + ${openedQuantity} açıq`;
                                    ...
                                    {saleItems.map((saleItem) => {
                                        ...
                                        const formatQuantity = (quantity) => {
                                            ...
                                            if (boxes > 0 && pieces > 0) {
                                                ...
                                                return `${boxes} ədəd ${boxUnit} daxilində (${quantity} ${unitLabel})`;
                                            } else if (pieces > 0) {
                                        ...
                                        // Qaytarılan məbləği hesabla
                                        const calculateReturnAmount = () => {
                                            if (!returnQty || returnQty <= 0) return 0;
                                            return calculateProductPrice(product, returnQty, saleItem.pricePerItem);
                                        };
                                                    ...
                                                    <div className="flex-1">
                                                        ...
                                                        {returnQty > 0 && returnAmount > 0 && (
                                                            <p className="text-xs text-red-600 font-semibold mt-1">
                                                                {t('returned_amount') || 'Qaytarılan məbləğ'}: {returnAmount.toFixed(2)} ₼
                                                            </p>
                                                        )}
                                                    </div>
                                                            ...
                                                            <>
                                                                <div className="flex items-center gap-2">
                                                                    ...
                                                                    <input
                                                                        ...
                                                                        onChange={(e) => {
                                                                            const maxBoxes = Math.floor(availableQty / product.piecesPerBox);
                                                                            let value = e.target.value;
                                                                            if (value !== '' && parseInt(value) > maxBoxes) {
                                                                                value = maxBoxes.toString();
                                                                            }
                                                                            handleReturnBoxesChange(saleItem.id, value);
                                                                        }}
                                                                        onKeyPress={(e) => {
                                                                            const char = String.fromCharCode(e.which);
                                                                            if (!/[0-9]/.test(char)) {
                                                                ...
                                                                <div className="flex items-center gap-2">
                                                                    ...
                                                                    <input
                                                                        ...
                                                                        onChange={(e) => {
                                                                            const returnItem = returnItems.find(ri => ri.saleItemId === saleItem.id);
                                                                            const boxesNum = returnItem?.boxes ? parseInt(returnItem.boxes) : 0;
                                                                            const maxPieces = availableQty - (boxesNum * product.piecesPerBox);
                                                                            let value = e.target.value;
                                                                            if (value !== '' && parseInt(value) > maxPieces) {
                                                                                value = maxPieces.toString();
                                                                            }
                                                                            handleReturnPiecesChange(saleItem.id, value);
                                                                        }}
                                                                        onKeyPress={(e) => {
                                                                            const char = String.fromCharCode(e.which);
                                                                            if (!/[0-9]/.test(char)) {
